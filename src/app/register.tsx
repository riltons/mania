...
export default function Register() {
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            console.log('Chamando signUp com:', form.email, '********', form.fullName);
            const { data, error: signUpError } = await signUp(form.email, form.password, form.fullName);
            
            if (signUpError) {
                console.error('Erro retornado pelo signUp:', signUpError);
            ...
            let errorMessage = typeof error === 'string' ? error : error?.message || 'Não foi possível criar sua conta. Tente novamente.';
...
const Button = styled.TouchableOpacity`
    background-color: ${({ theme, disabled }: ButtonProps) => disabled ? theme.colors.primary + '80' : theme.colors.primary};
    border-radius: 8px;
    padding: 16px;
    align-items: center;
    margin-top: 24px;
