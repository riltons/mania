...
CREATE POLICY IF NOT EXISTS "Usuários podem ver suas próprias assinaturas"
ON subscriptions
FOR SELECT
USING (auth.uid() = user_id);

-- Política para usuários atualizarem apenas suas próprias assinaturas
CREATE POLICY IF NOT EXISTS "Usuários podem atualizar suas próprias assinaturas"
ON subscriptions
FOR UPDATE
USING (auth.uid() = user_id);

-- Política para permitir inserção de assinaturas pelo próprio usuário
CREATE POLICY IF NOT EXISTS "Usuários podem inserir suas próprias assinaturas"
ON subscriptions
FOR INSERT
WITH CHECK (auth.uid() = user_id);

-- Política para permitir que o serviço de backend acesse todas as assinaturas
CREATE POLICY IF NOT EXISTS "Service role pode acessar todas as assinaturas"
ON subscriptions
FOR ALL
