// Script para aplicar migração SQL ao Supabase usando SQL direto
const fs = require('fs');
...
async function aplicarMigracao() {
  try {
    ...
    console.log('\nTentando aplicar a migração usando SQL direto...');
    ...
    for (let i = 0; i < sqlCommands.length; i++) {
      ...
      try {
        ...
        const { data, error } = await supabase.from('_sql').select('*').execute(command);
